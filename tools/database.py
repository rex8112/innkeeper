...
def saveAdventurer(save):
  cursor.execute( """UPDATE adventurers SET name = ?, class = ?, level = ?, xp = ?, race = ?, attributes = ?, skills = ?, equipment = ?, inventory = ?, available = ?, health = ? WHERE id = ?""", (save[1], save[2], save[3], save[4], save[5], save[6], save[7], save[8], save[9], save[10], save[11], save[0]))
  db.commit()
  return save[0]
...
def addEnemy(name, cls, race, attributes, skills, rng):
  cursor2.execute( """INSERT INTO enemies(name, class, level, xp, race, attributes, skills, rng) VALUES(?, ?, ?, ?, ?, ?, ?, ?)""", (name, cls, 1, 0, race, attributes, skills, rng))
  db2.commit()
...
def getEnemyRNG(lvl: int, offset = 0, rnge = 3):
  maximum = lvl + offset
  minimum = maximum - rnge
  cursor2.execute( """SELECT * FROM enemies WHERE rng = 1 AND level BETWEEN ? AND ?""", (minimum, maximum))
...
def saveRNG(save):
  if save[0] == 0:
    save[0] = addRNG()
  cursor.execute( """UPDATE rngdungeons SET adv = ?, active = ?, stage = ?, stages = ?, enemies = ?, loot = ?, time = ?, xp = ? WHERE indx = ?""", (save[1], save[2], save[3], save[4], save[5], save[6], save[7], save[8], save[0]))
  db.commit()
  return save[0]
...
def getTimeRNG():
  cursor.execute( """SELECT * FROM rngdungeons WHERE active = 1 AND time <= datetime('now', 'localtime')""" )
